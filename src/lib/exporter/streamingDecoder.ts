...
const EPSILON_SEC = EXPORT_TIMELINE_EPSILON_SEC;
/**
...
export class StreamingVideoDecoder {
	...
	async decodeAll(
		...
		const shouldPreferSoftwareDecode =
			...
			codec.includes("vp09") ||
			codec.includes("vp9");
		const segments = buildExportTimelineSegments(this.metadata.duration, trimRegions, speedRegions);
	...
	getExportMetrics(
		...
		return getExportTimelineMetrics(
