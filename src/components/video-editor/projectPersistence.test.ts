	...
	it("merges overlapping trims but preserves adjacent cuts when loading a project", () => {
		const editor = normalizeProjectEditor({
			trimRegions: [
				...
				{ id: "touching", startMs: 200, endMs: 300 },
			],
		});
...
describe("wallpaper legacy normalization", () => {
	...
	it("rewrites new packaged layout (resources/wallpapers/…)", () => {
		const normalized = normalizeProjectEditor({
			wallpaper: "file:///opt/Videtio/resources/wallpapers/wallpaper3.jpg",
		});
	...
	it("falls back to default for bundled paths outside WALLPAPER_PATHS", () => {
		const normalized = normalizeProjectEditor({
			wallpaper: "file:///opt/Videtio/resources/wallpapers/wallpaper99.jpg",
		});
		expect(normalized.wallpaper).toBe("/wallpapers/wallpaper1.jpg");
	});
});
