...
const OUTPUT_DIR =
	...
	path.join(os.tmpdir(), `videtio-cursor-native-${Date.now()}`);
...
function spawnPowerShell(script, { onStdout, onStderr } = {}) {
	const scriptPath = path.join(
		os.tmpdir(),
		`videtio-powershell-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}.ps1`,
	);
...
public static class VidetioCursorDiagnosticInterop {
...
$standardCursors = @{
    arrow = [VidetioCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32512))
    text = [VidetioCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32513))
    wait = [VidetioCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32514))
    crosshair = [VidetioCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32515))
    'up-arrow' = [VidetioCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32516))
    'resize-nwse' = [VidetioCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32642))
    'resize-nesw' = [VidetioCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32643))
    'resize-ew' = [VidetioCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32644))
    'resize-ns' = [VidetioCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32645))
    move = [VidetioCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32646))
    'not-allowed' = [VidetioCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32648))
    pointer = [VidetioCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32649))
    'app-starting' = [VidetioCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32650))
    help = [VidetioCursorDiagnosticInterop]::LoadCursor([IntPtr]::Zero, [IntPtr]::new(32651))
}
...
function Get-CursorAsset($cursorHandle, $cursorId) {
    $copiedHandle = [VidetioCursorDiagnosticInterop]::CopyIcon($cursorHandle)
    if ($copiedHandle -eq [IntPtr]::Zero) {
        return $null
    }

    $iconInfo = New-Object VidetioCursorDiagnosticInterop+ICONINFO
    $hasIconInfo = [VidetioCursorDiagnosticInterop]::GetIconInfo($copiedHandle, [ref]$iconInfo)
    ...
    try {
        ...
        if ($hasIconInfo) {
            if ($iconInfo.hbmMask -ne [IntPtr]::Zero) {
                [VidetioCursorDiagnosticInterop]::DeleteObject($iconInfo.hbmMask) | Out-Null
            }
            if ($iconInfo.hbmColor -ne [IntPtr]::Zero) {
                [VidetioCursorDiagnosticInterop]::DeleteObject($iconInfo.hbmColor) | Out-Null
            }
        }
        [VidetioCursorDiagnosticInterop]::DestroyIcon($copiedHandle) | Out-Null
    }
}

[VidetioCursorDiagnosticInterop]::InstallMouseHook() | Out-Null
[VidetioCursorDiagnosticInterop]::GetAsyncKeyState(0x01) | Out-Null
...
while ($true) {
    ...
    $mouseEvents = [VidetioCursorDiagnosticInterop]::ConsumeMouseButtonEvents()
    $cursorInfo = New-Object VidetioCursorDiagnosticInterop+CURSORINFO
    $cursorInfo.cbSize = [Runtime.InteropServices.Marshal]::SizeOf([type][VidetioCursorDiagnosticInterop+CURSORINFO])

    if (-not [VidetioCursorDiagnosticInterop]::GetCursorInfo([ref]$cursorInfo)) {
    ...
    $leftButtonState = [VidetioCursorDiagnosticInterop]::GetAsyncKeyState(0x01)
...
public static class VidetioMouseDiagnosticInterop {
    [DllImport("user32.dll")]
    [return: MarshalAs(UnmanagedType.Bool)]
...
for ($i = 0; $i -lt $points.Count; $i++) {
    $point = $points[$i]
    [VidetioMouseDiagnosticInterop]::SetCursorPos($point.x, $point.y) | Out-Null
    if ($i -eq [int]([Math]::Floor($points.Count / 2))) {
        [VidetioMouseDiagnosticInterop]::mouse_event(0x0002, 0, 0, 0, [UIntPtr]::Zero)
        Start-Sleep -Milliseconds 12
        [VidetioMouseDiagnosticInterop]::mouse_event(0x0004, 0, 0, 0, [UIntPtr]::Zero)
    }
...
<title>Videtio native cursor diagnostic</title>
<style>
...
<body>
<main>
<h1>Videtio native cursor diagnostic</h1>
<div class="metrics">
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Videtio native cursor real capture diagnostic</title>
<style>
