...
const OUTPUT_DIR =
	process.env.VIDETIO_PREVIEW_OUTPUT_DIR ??
	path.join(os.tmpdir(), `videtio-real-preview-${Date.now()}`);
const FRAME_COUNT = Number(process.env.VIDETIO_PREVIEW_FRAME_COUNT ?? 90);
const FPS = Number(process.env.VIDETIO_PREVIEW_FPS ?? 30);
...
function findLatestCursorRecordingData() {
	...
	const candidates = fs
		...
		.filter((entry) => entry.isDirectory() && entry.name.startsWith("videtio-cursor-native-"))
...
function runNpmBuildViteIfRequested() {
	if (process.env.VIDETIO_PREVIEW_SKIP_BUILD === "true") {
		ensureBuildExists();
...
const fixtureVideoPath = path.join(OUTPUT_DIR, "videtio-preview-fixture.webm");
const outputVideoPath = path.join(OUTPUT_DIR, "videtio-preview.webm");
...
try {
	...
	await hudWindow.evaluate(async () => {
		...
		throw new Error("Timed out waiting for Videtio IPC handlers.");
	});
